...
IPAddress::ptr Address::LookupAnyIPAddress(const std::string &host, int family, int type, int protocal) {
    std::vector<Address::ptr> result;
    if(Lookup(result, host, family, type, protocal)) {
        for(auto& i : result) {
...
bool Address::Lookup(std::vector<Address::ptr> &result, const std::string &host, int family, int type, int protocal) {
    ...
    if(error) {
        ZERO_LOG_INFO(g_logger) << "Address::Lookup getaddress(" << host << ", "
...
bool Address::GetInterfaceAddresses(std::multimap<std::string, std::pair<Address::ptr, uint32_t>>& result, int family) {
    ...
    if(getifaddrs(&results) != 0) {
        ZERO_LOG_INFO(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
        ...
        for(next = results; next; next = next->ifa_next) {
            ...
            switch (next->ifa_addr->sa_family) {
                ...
                case AF_INET6:
                    {
                        addr = Create(next->ifa_addr, sizeof(sockaddr_in));
                        in6_addr& nextmask = ((sockaddr_in6*)next->ifa_netmask)->sin6_addr;
                        prefix_len = 0;
                        for(int i = 0;i < 16; ++i) {
                            prefix_len += CountBytes(nextmask.s6_addr[i]);
                        }
                    }
                    break;
                default:
...
/// 即便v4和v6结构体大小不一致，但是只要拿到地址和长度，就可以找到具体结构
...
IPAddress::ptr IPAddress::Create(const char *address, uint16_t port) {
    ...
    /// 此处存疑，address是不是只能设置为数字字符串类型?
    hints.ai_flags = AI_NUMERICHOST;
    hints.ai_family = AF_UNSPEC;

    int error = getaddrinfo(address, NULL, &hints, &results);
    if(error) {
        ZERO_LOG_INFO(g_logger) << "IPAddress::Create(" << address
            << ", " << port << ") error=" << error
...
IPv4Address::ptr IPv4Address::Create(const char *address, uint16_t port) {
    ...
    /// 将本机字节序转换为网络字节序，即由小端转换为大端
    rt->m_addr.sin_port = byteswapOnLittleEndian(port);
    int result = inet_pton(AF_INET, address, &rt->m_addr.sin_addr);
    if(result <= 0) {
        ZERO_LOG_INFO(g_logger) << "IPv4Address::Create(" << address << ", "
...
IPv6Address::ptr IPv6Address::Create(const char *address, uint16_t port) {
    ...
    if(result <= 0) {
        ZERO_LOG_INFO(g_logger) << "IPv6Address::Create(" << address << ", "
                << port << ") rt=" << result << " errno=" << errno
                << " errstr=" << strerror(errno);
        return nullptr;        
    }
